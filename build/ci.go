...
import (
	...
	"github.com/gameconnection/gamefin_network/crypto/signify"
	"github.com/gameconnection/gamefin_network/internal/build"
...
var (
	...
	debExecutables = []debExecutable{
		{
			BinaryName:  "abigen",
			Description: "Source code generator to convert Gamefin contract definitions into easy to use, compile-time type-safe Go packages.",
		},
		{
			BinaryName:  "bootnode",
			Description: "Gamefin bootnode.",
		},
		{
			BinaryName:  "evm",
			Description: "Developer utility version of the EVM (Gamefin Virtual Machine) that is capable of running bytecode snippets within a configurable environment and execution mode.",
		},
		{
			BinaryName:  "geth",
			Description: "Gamefin CLI client.",
		},
		{
			BinaryName:  "puppeth",
			Description: "Gamefin private network manager.",
		},
		{
			BinaryName:  "rlpdump",
		...
		{
			BinaryName:  "clef",
			Description: "Gamefin account management tool.",
		},
	}

	// A debian package is created for all executables listed here.
	debGamefin = debPackage{
		Name:        "gamefin",
...
func doDocker(cmdline []string) {
	var (
		...
		upload   = flag.String("upload", "", `Where to upload the docker image (usually "gamefin/client-go")`)
	)
	flag.CommandLine.Parse(cmdline)
	...
	//  - gamefin/client-go:latest                            - Pushes to the master branch, Geth only
	//  - gamefin/client-go:stable                            - Version tag publish on GitHub, Geth only
	//  - gamefin/client-go:alltools-latest                   - Pushes to the master branch, Geth & tools
	//  - gamefin/client-go:alltools-stable                   - Version tag publish on GitHub, Geth & tools
	//  - gamefin/client-go:release-<major>.<minor>           - Version tag publish on GitHub, Geth only
	//  - gamefin/client-go:alltools-release-<major>.<minor>  - Version tag publish on GitHub, Geth & tools
	//  - gamefin/client-go:v<major>.<minor>.<patch>          - Version tag publish on GitHub, Geth only
	//  - gamefin/client-go:alltools-v<major>.<minor>.<patch> - Version tag publish on GitHub, Geth & tools
	var tags []string
...
func doDebianSource(cmdline []string) {
	var (
		...
		upload   = flag.String("upload", "", `Where to upload the source package (usually "gamefin/gamefin")`)
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, pkg := range debPackages {
		for distro, goboot := range debDistroGoBoots {
			// Prepare the debian package with the go-gamefin sources.
...
type debPackage struct {
	Name        string          // the name of the Debian package to produce, e.g. "gamefin"
...
type debMetadata struct {
	...
	// go-gamefin version being built. Note that this
...
func newDebMetadata(distro, goboot, author string, env build.Environment, t time.Time, name string, version string, exes []debExecutable) debMetadata {
	if author == "" {
		...
		author = "Gamefin Builds <fjl@gamefin.org>"
	}
	return debMetadata{
...
func doAndroidArchive(cmdline []string) {
	...
	build.MustRun(gomobileTool("bind", "-ldflags", "-s -w", "--target", "android", "--javapkg", "org.gamefin", "-v", "github.com/gameconnection/gamefin_network/mobile"))
...
func doXCodeFramework(cmdline []string) {
	...
	bind := gomobileTool("bind", "-ldflags", "-s -w", "--target", "ios", "-v", "github.com/gameconnection/gamefin_network/mobile")
