...
func newClientHandler(ulcServers []string, ulcFraction int, checkpoint *params.TrustedCheckpoint, backend *LightGamefin) *clientHandler {
...
func (h *clientHandler) handle(p *serverPeer, noInitAnnounce bool) error {
	...
	p.Log().Debug("Light Gamefin peer connected", "name", p.Name())
	...
	if err := p.Handshake(h.backend.blockchain.Genesis().Hash(), forkid, h.forkFilter); err != nil {
		p.Log().Debug("Light Gamefin handshake failed", "err", err)
		return err
	}
	...
	if err := h.backend.peers.register(p); err != nil {
		p.Log().Error("Light Gamefin peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := h.handleMsg(p); err != nil {
			p.Log().Debug("Light Gamefin message handling failed", "err", err)
			p.fcServer.DumpLogs()
			return err
		}
	}
...
func (h *clientHandler) handleMsg(p *serverPeer) error {
	...
	p.Log().Trace("Light Gamefin message arrived", "code", msg.Code, "bytes", msg.Size)
