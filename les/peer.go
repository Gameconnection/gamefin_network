...
import (
	...
	"github.com/gameconnection/gamefin_network/les/flowcontrol"
	"github.com/gameconnection/gamefin_network/les/utils"
	vfc "github.com/gameconnection/gamefin_network/les/vflux/client"
	vfs "github.com/gameconnection/gamefin_network/les/vflux/server"
...
type PeerInfo struct {
	Version    int      `json:"version"`    // Gamefin protocol version negotiated
...
func (p *peerCommons) handshake(td *big.Int, head common.Hash, headNum uint64, genesis common.Hash, forkID forkid.ID, forkFilter forkid.Filter, sendCallback func(*keyValueList), recvCallback func(keyValueMap) error) error {
	...
	// as well. Check http://eips.gamefin.org/EIPS/eip-2124 for more
	// spec detail.
	if p.version >= lpv4 {
	...
	return p.handshake(td, head, headNum, genesis, forkID, forkFilter, func(lists *keyValueList) {
		...
		if !server.config.UltraLightOnlyAnnounce {
			...
			*lists = (*lists).add("serveStateSince", uint64(0))
			...
			// If local gamefin node is running in archive mode, advertise ourselves we have
...
// participating in the Light Gamefin sub-protocol.
