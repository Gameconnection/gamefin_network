...
import (
	...
	"github.com/gameconnection/gamefin_network/accounts/usbwallet/trezor"
...
// or whether the Gamefin app was not started on it.
...
// Gamefin address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.GamefinAddress)
	if _, err := w.trezorExchange(&trezor.GamefinGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.GamefinTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.GamefinTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
