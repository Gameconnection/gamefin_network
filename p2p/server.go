...
// Package p2p implements the Gamefin p2p network protocols.
package p2p
...
import (
	...
	"github.com/gameconnection/gamefin_network/p2p/discover"
...
func (srv *Server) setupDiscovery() error {
	...
	if srv.NAT != nil {
		if !realaddr.IP.IsLoopback() {
			srv.loopWG.Add(1)
			go func() {
				nat.Map(srv.NAT, srv.quit, "udp", realaddr.Port, realaddr.Port, "gamefin discovery")
				srv.loopWG.Done()
			}()
		}
	}
...
func (srv *Server) setupListening() error {
	...
	if tcp, ok := listener.Addr().(*net.TCPAddr); ok {
		...
		if !tcp.IP.IsLoopback() && srv.NAT != nil {
			srv.loopWG.Add(1)
			go func() {
				nat.Map(srv.NAT, srv.quit, "tcp", tcp.Port, tcp.Port, "gamefin p2p")
				srv.loopWG.Done()
			}()
		}
	}
...
type NodeInfo struct {
	...
	ENR   string `json:"enr"`   // Gamefin Node Record
