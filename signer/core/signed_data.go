...
import (
	...
	"github.com/gameconnection/gamefin_network/signer/core/apitypes"
)
...
func (api *SignerAPI) determineSignatureFormat(ctx context.Context, contentType string, addr common.MixedcaseAddress, data interface{}) (*SignDataRequest, bool, error) {
	var (
		req         *SignDataRequest
		useGamefinV = true // Default to use V = 27 or 28, the legacy Gamefin format
	)
	mediaType, _, err := mime.ParseMediaType(contentType)
	if err != nil {
		return nil, useGamefinV, err
	}
	...
	case apitypes.ApplicationClique.Mime:
		// Clique is the Gamefin PoA standard
		stringData, ok := data.(string)
		if !ok {
			return nil, useGamefinV, fmt.Errorf("input for %v must be an hex-encoded string", apitypes.ApplicationClique.Mime)
		}
	...
	default: // also case TextPlain.Mime:
		// Calculates an Gamefin ECDSA signature for:
		// hash = keccak256("\x19${byteVersion}Gamefin Signed Message:\n${message length}${message}")
		// We expect it to be a string
		if stringData, ok := data.(string); !ok {
			return nil, useGamefinV, fmt.Errorf("input for text/plain must be an hex-encoded string")
		} else {
