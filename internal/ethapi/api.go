// Copyright 2015 The go-gamefin Authors
...
import (
	...
	"github.com/gameconnection/gamefin_network/accounts/abi"
	...
	"github.com/gameconnection/gamefin_network/eth/tracers/logger"
...
// PublicGamefinAPI provides an API to access Gamefin related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicGamefinAPI struct {
	b Backend
}

// NewPublicGamefinAPI creates a new Gamefin protocol API.
func NewPublicGamefinAPI(b Backend) *PublicGamefinAPI {
	return &PublicGamefinAPI{b}
}

// GasPrice returns a suggestion for a gas price for legacy transactions.
func (s *PublicGamefinAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
...
func (s *PublicGamefinAPI) MaxPriorityFeePerGas(ctx context.Context) (*hexutil.Big, error) {
	tipcap, err := s.b.SuggestGasTipCap(ctx)
	if err != nil {
		return nil, err
	}
...
func (s *PublicGamefinAPI) FeeHistory(ctx context.Context, blockCount rpc.DecimalOrHex, lastBlock rpc.BlockNumber, rewardPercentiles []float64) (*feeHistoryResult, error) {
...
func (s *PublicGamefinAPI) Syncing() (interface{}, error) {
...
// Sign calculates an Gamefin ECDSA signature for:
// keccack256("\x19Gamefin Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/gameconnection/gamefin_network/wiki/Management-APIs#personal_sign
...
// the address of:
// hash = keccak256("\x19Gamefin Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be 27 or 28 for legacy reasons.
//
// https://github.com/gameconnection/gamefin_network/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != crypto.SignatureLength {
		return common.Address{}, fmt.Errorf("signature must be %d bytes long", crypto.SignatureLength)
	}
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid Gamefin signature (V is not 27 or 28)")
	}
...
// PublicBlockChainAPI provides an API to access the Gamefin blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new Gamefin blockchain API.
func NewPublicBlockChainAPI(b Backend) *PublicBlockChainAPI {
	return &PublicBlockChainAPI{b}
}

// ChainId is the EIP-155 replay-protection chain id for the current gamefin chain config.
...
// See: https://github.com/gamefin/wiki/wiki/JSON-RPC-Error-Codes-Improvement-Proposal
...
//
// https://github.com/gamefin/wiki/wiki/JSON-RPC#eth_sign
...
// PublicDebugAPI is the collection of Gamefin APIs exposed over the public
// debugging endpoint.
...
// PrivateDebugAPI is the collection of Gamefin APIs exposed over the private
// debugging endpoint.
...
// Version returns the current gamefin protocol version.
