...
// Package geth contains the simplified mobile APIs to go-gamefin.
//
// The scope of this package is *not* to allow writing a custom Gamefin client
// with pieces plucked from go-gamefin, rather to allow writing native dapps on
...
// from the go-gamefin repository. Please adhere to this scoping to prevent this
// package getting unmaintainable.
//
// Wrapping guidelines:
//
// Every type that is to be exposed should be wrapped into its own plain struct,
// which internally contains a single field: the original go-gamefin version.
