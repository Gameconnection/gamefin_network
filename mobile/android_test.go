...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.gamefin", new NodeConfig());
			node.start();
			...
			GamefinClient ec = node.getGamefinClient();
	...
	// signatures too. Regression test for go-gamefin issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/gamefin/gethtest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.gamefin", "github.com/gameconnection/gamefin_network/mobile")
	...
	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "gamefin", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
