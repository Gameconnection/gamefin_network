...
// Verify that Client implements the gamefin interfaces.
var (
	_ = gamefin.ChainReader(&Client{})
	_ = gamefin.TransactionReader(&Client{})
	_ = gamefin.ChainStateReader(&Client{})
	_ = gamefin.ChainSyncReader(&Client{})
	_ = gamefin.ContractCaller(&Client{})
	_ = gamefin.GasEstimator(&Client{})
	_ = gamefin.GasPricer(&Client{})
	_ = gamefin.LogFilterer(&Client{})
	_ = gamefin.PendingStateReader(&Client{})
	// _ = gamefin.PendingStateEventer(&Client{})
	_ = gamefin.PendingContractCaller(&Client{})
)
...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	if err == nil || err == gamefin.NotFound {
		t.Fatal("error should not be nil/notfound")
	}

	// Test tx in block not found.
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 20); err != gamefin.NotFound {
		t.Fatal("error should be gamefin.NotFound")
	}
}
