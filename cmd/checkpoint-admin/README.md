...
*For a more detailed introduction to checkpoint, please see the LES [spec](https://github.com/gamefin/devp2p/blob/master/caps/les.md).*
...
There are several hardcoded checkpoints in the [source code](https://github.com/gameconnection/gamefin_network/blob/master/params/config.go#L38) of the go-gamefin project. These checkpoints are updated by go-gamefin developers when new versions of software are released. Because light client users trust Geth developers to some extent, hardcoded checkpoints in the code can also be considered correct.
...
```shell
go get github.com/gameconnection/gamefin_network/cmd/checkpoint-admin
```
...
It is worth noting that checkpoint-admin only supports clef as a signer for transactions and plain text(checkpoint). For more clef usage, please see the clef [tutorial](https://geth.gamefin.org/docs/clef/tutorial) .
...
*CHECKPOINT_HASH is obtained based on this [calculation method](https://github.com/gameconnection/gamefin_network/blob/master/params/config.go#L251).*
