...
// nodeDockerfile is the Dockerfile required to run an Gamefin node.
var nodeDockerfile = `
FROM gamefin/client-go:latest
...
RUN \
  echo 'geth --cache 512 init /genesis.json' > geth.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.gamefin/keystore/ && cp /signer.json /root/.gamefin/keystore/' >> geth.sh && \{{end}}
...
// an Gamefin node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.gamefin{{if .Ethashdir}}
...
// deployNode deploys a new Gamefin node container to a remote machine via SSH,
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.gamefin"],
